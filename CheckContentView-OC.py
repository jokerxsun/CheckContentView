...
def matchATSMethod(dir_path, rematch):
    suf_set = ('.h') # 只需要遍历 .h 文件，就可以确定，是否是 UITableViewCell、UICollectionViewCell 类
...
# 打开 .m 文件，遍历每一行，是否符合 [self addSubviews:] 格式，符合输出当前类名、以及所在行
def checkATSMethod(root, file_name, match):
    ats_symbol_containing = '[self addSubview:'
...
# 直接指定源码根路径就可以, 以及想要排查的类
matchATSMethod('/Users/xx/xx/xx/xx/Pods', 'UICollectionViewCell') 
matchATSMethod('/Users/xx/xx/xx/xx/Pods', 'UITableViewCell')            